...
Zero_elim <- function(B){
  ...
  # Get and Remove All Zero Rows Or Columns
  for(i in 1:m_rows){
    if(all(B[i,] == 0)){
      Z_rows <- c(Z_rows, i)
    }
...
Gaussian_rref <- function(A, simplify = FALSE){
  ...
  for(i in 1:(new_m_rows - 1)){
    for(j in (i + 1):new_m_rows){
      if(Linear_check(B[i,], B[j,])){
        B[j,] <- rep(0, m_cols) # Set duplicates to 0 vectors
      }
    }
  }
  ...
  if(is.null(new_m_rows)){
    return("All Rows Are The Same: Pick Some Variables And
